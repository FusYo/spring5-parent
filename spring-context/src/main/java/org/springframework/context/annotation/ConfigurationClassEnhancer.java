...
class ConfigurationClassEnhancer {
	...
	public Class<?> enhance(Class<?> configClass, @Nullable ClassLoader classLoader) {
		...
		//返回一个cglib class
		//newEnhancer(configClass, classLoader) 实现cglib代理
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		...
		//设置cglib代理的父类,其实就是 加了@Configuration的类
		enhancer.setSuperclass(configSuperClass);
		//增强接口(就是给代理的类添加多一个接口),表示一个类已经被增强了
		//也就是说代理类被添加了多接口,意味着多了方法,就是增强了
		enhancer.setInterfaces(new Class<?>[] {EnhancedConfiguration.class});
		enhancer.setUseFactory(false);
		enhancer.setNamingPolicy(SpringNamingPolicy.INSTANCE);
		//给代理处理的类添加一个属性 &&beanFactory,因为增强方法中需要beanFactory
		//所以Spring在代理类中添加了一个属性 &&beanFactory用于获取beanFactory的值
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			//从 enhancedConfigInstance 代理对象获取 beanFactory
			...
			/**
			 *
			 首先，检查请求的bean是否是factorybean。如果是，那么创建一个子类代理，
			 拦截对getobject（）的调用并返回任何缓存的bean实例，
			 这样可以确保从@bean方法中调用factorybean的语义与在xml中引用factorybean的语义相同。
			 */
			if (factoryContainsBean(beanFactory, BeanFactory.FACTORY_BEAN_PREFIX + beanName) &&
					factoryContainsBean(beanFactory, beanName)) {
				//	String FACTORY_BEAN_PREFIX = "&";从工厂中拿出 带有"&" 符号的beanName;
				...
				if (factoryBean instanceof ScopedProxyFactoryBean) {
					...
					//当前的bean是一个Factorybean的话,getObject(),返回的对象又进行一次代理
					return enhanceFactoryBean(factoryBean, beanMethod.getReturnType(), beanFactory, beanName);
				}
			}

			//判断方法里面时候还有调用方法,如果调用了其他的方法返回false
			//如果返回的是true的话,则调用父类的方法
			//method.invoke.super() 父类是原来的方法
			if (isCurrentlyInvokedFactoryMethod(beanMethod)) {
				// The factory is calling the bean method in order to instantiate and register the bean
				// (i.e. via a getBean() call) -> invoke the super implementation of the method to actually
				// create the bean instance.
				/**
				 * 工厂调用bean方法是为了实例化和注册bean（即通过getBean（）调用）->
				 * 调用方法的超级实现来实际创建bean实例。
				 */

...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		//如果你想在扫描所有包之前就执行你的后置处理器的话,可以收下把的后置处理器注册该Spring先
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			//2. 处理Spring内部的后置处理器  Spring内部的处理器 ,就只有一个类实现了BeanDefinitionRegistryPostProcessor
			...
			//其实是几乎不可能的,Spring保障一点嘛!1
			postProcessorNames = beanFactory.getBeanNamesForType(BeanDefinitionRegistryPostProcessor.class, true, false);
			for (String ppName : postProcessorNames) {
				//不会进去的 因为上面 我们 processedBeans.add(ppName);  把
		...
		//下面的代码时执行用户自己定义的后置处理器,重点在这里
		//因为所有的包都被扫描出来了,我们用户自定义的后置处理器可以开始执行了
	...
	public static void registerBeanPostProcessors(
		...
		// AutoWiredAnnotationBeanPostProcessor   CommonAnnotationBeanPostProcessor
         //如果加了启用AOP的注解@EnableAspectJAutoProxy 在这里也会 出现AnnotationAwareAspectAutoProxyCreator
